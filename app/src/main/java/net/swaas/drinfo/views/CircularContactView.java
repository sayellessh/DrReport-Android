...
public class CircularContactView extends ViewSwitcher {
    ...
    public CircularContactView(final Context context, final AttributeSet attrs) {
        ...
        mContentSize = getResources().getDimensionPixelSize(R.dimen.imageview_size);
        if (isInEditMode())
    ...
    private void drawContent(final int viewWidth, final int viewHeight) {
        ...
        if (mImageResId != 0) {
            ...
            int pixSize = getContext().getResources().getDimensionPixelSize(R.dimen.text_size_caption_material);
            mTextView.setTextSize(pixSize);
